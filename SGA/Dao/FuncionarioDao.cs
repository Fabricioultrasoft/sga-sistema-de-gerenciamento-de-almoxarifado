...
namespace SGA.Dao
{
    class FuncionarioDao
    {
        ...
        public Queue pesquisaFuncionario(Funcionario func)
        {
            ...
            if (func.periodo[0] + "" != "")
            {
                codigo = codigo + " AND dt_ins_funcionario BETWEEN '" + Convert.ToDateTime(Convert.ToDateTime(func.periodo[0]).ToString("yyyy-MM-dd HH:mm:ss")) + "' AND '" + Convert.ToDateTime(Convert.ToDateTime(func.periodo[1]).ToString("yyyy-MM-dd HH:mm:ss")) + "'";
            }
